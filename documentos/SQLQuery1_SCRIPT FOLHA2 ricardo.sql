...
CREATE TABLE FPEMP (
    ...
    ce_NFILIAL FLOAT,                           -- Número da filial (FLOAT)
    ce_SIGEMP  VARCHAR(2),                      -- Sigla da empresa (2 caracteres)
    ce_CODIGO  FLOAT,                           -- Código da empresa (FLOAT)
    ...
    CONSTRAINT PK_FPEMP_ID PRIMARY KEY (ID),

    -- Vínculo com a própria tabela FPEMP pela empresa e filial
    CONSTRAINT FK_FPEMP_SELF FOREIGN KEY (ce_NUMEMP, ce_NFILIAL)  REFERENCES FPEMP(ce_NUMEMP, ce_NFILIAL)
);
GO
...
CREATE TABLE FPCD (
    ...
    CD_NUMEMP        FLOAT,                        -- Número da empresa (ligação com FPEMP)
    CD_NFILIAL       FLOAT,                        -- Número da filial (ligação com FPEMP)
    
    -- Vínculos:
    CONSTRAINT PK_FPCD PRIMARY KEY (CD_NUMMAT),           -- Chave primária na matrícula
    CONSTRAINT UNQ_FPCD_CPF UNIQUE (CD_NUMCPF),           -- CPF deve ser único
    CONSTRAINT CHK_FPCD_SEXO CHECK (CD_SEXO IN('M','F')), -- Restrição para sexo masculino ou feminino
    CONSTRAINT CHK_FPCD_CD_SALMES CHECK (CD_SALMES > 0),  -- Verifica se o salário é maior que zero

    -- Vínculo com a tabela FPEMP (Empresa e Filial)
    CONSTRAINT FK_FPCD_FPEMP FOREIGN KEY (CD_NUMEMP, CD_NFILIAL) REFERENCES FPEMP(ce_NUMEMP, ce_NFILIAL),

    -- Vínculo com a tabela FPCF (Dados Complementares de Funcionário)
    CONSTRAINT FK_FPCD_FPCF FOREIGN KEY (CD_NUMMAT) REFERENCES FPCF(CF_NUMMAT),

    -- Outros vínculos com tabelas auxiliares:
    CONSTRAINT FK_FPCD_FPLT FOREIGN KEY (CD_NUMLOT) REFERENCES FPLT(LT_NUMLOT),
    CONSTRAINT FK_FPCD_FPRL FOREIGN KEY (CD_NUMMAT) REFERENCES FPRL(RL_NUMMAT),
    CONSTRAINT FK_FPCD_FPRLS FOREIGN KEY (CD_NUMMAT) REFERENCES FPRLS(ID_RLSMAT),
    CONSTRAINT FK_FPCD_FPRLD FOREIGN KEY (CD_NUMMAT) REFERENCES FPRLD(ID_RLDMAT),
    CONSTRAINT FK_FPCD_FPBA FOREIGN KEY (CD_BCOCTA) REFERENCES FPBA(BA_CODBCO),
    CONSTRAINT FK_FPCD_FPDF FOREIGN KEY (CD_NUMMAT) REFERENCES FPDF(DF_NUMMAT),
    CONSTRAINT FK_FPCD_FPRES FOREIGN KEY (CD_NUMMAT) REFERENCES FPRES(VA_NUMMAT)
);
GO
...
--SQL
ALTER TABLE FPCD 
ADD CD_MOTIVO_DESLIGAMENTO VARCHAR(1); -- Motivo do desligamento (D: Demissão, A: Aposentadoria, etc.)
...
--------------------------------------------------------------------------------------------------------------------
--Obrigacoes 
--continua SEENDO USADA VIA FOLHA DE PAGAMENTO ]
--IRRF: DARF (via folha de pagamento)
...
CREATE TABLE FPVE (
    ...
    IDFPVE   INT IDENTITY(1,1),                  -- Identificador específico da tabela FPVE com auto-incremento
    MAT      VARCHAR(6),                         -- Número da matrícula do funcionário (até 6 caracteres)
    VER      VARCHAR(3),                         -- Código da verba (até 3 caracteres)
    DIA      VARCHAR(2),                         -- Dia do evento (até 2 caracteres)
    VAL      FLOAT,                              -- Valor da verba (FLOAT, substituindo NUMERIC(13,2))
    SIG      FLOAT,                              -- Sinal do valor, usado para indicar positivo ou negativo (FLOAT, substituindo NUMERIC(6,0))
    DOC      VARCHAR(10),                        -- Documento associado (até 10 caracteres)
    NUMEMP   FLOAT,                              -- Número da empresa (FLOAT, substituindo NUMERIC(2,0))
    NFILIAL  FLOAT,                              -- Número da filial (FLOAT, substituindo NUMERIC(4,0))
...
CREATE TABLE FPDF (
    ...
    -- Chave estrangeira para vincular com a tabela FPEMP (empresa e filial)
    CONSTRAINT FK_FPDF_FPEMP FOREIGN KEY (DF_NUMEMP, DF_NFILIAL)  REFERENCES FPEMP(ps_NUMEMP, ps_NFILIAL),
